...
if (isset($_POST['idDeportista'])) {
    $_SESSION['opcion'] = $_POST['idDeportista'];
}
$opcionSeleccionada = $_SESSION['opcion'] ?? '';
?>
        ...
        <?php if(isset($detalles)): ?>
            <?php if(isset($asistencias)): ?>
                ...
                <table id="tabla-ranking">
                    <tr>
                        <td colspan="4">
                            <p>El (La) deportista <?php echo $deportista ?> tuvo <?php echo $asistencias ?> asistencias, en las que obtuvo:</p>
                        </td>
                    </tr>
              ...
              <?php elseif(isset($eventos)): ?>
                <table id="tabla-ranking">
                    <tr>
                        <td colspan="5">
                            <p style="font-size: 0.8rem;">El(La) deportista <?php echo $deportista ?> ha participado en <?php echo $eventos?> competencias, con los siguientes resultados:</p>
                        </td>
                    </tr>
        ...
        <select name="idDeportista" id="idDeportista" required>
           <option value="">Elija el Deportista</option>
            ...
            foreach ($deportistas as $deportista) {
                $selected = ($deportista['id'] == $opcionSeleccionada) ? 'selected' : '';
                echo "<option value='{$deportista['id']}' $selected>{$deportista['nombres']} {$deportista['apellidos']}</option>";
            }
            ?>
        </select>
